...
当前 Design 是 doc-only：最终只落 `design.md` 和 `design.log`，不再落旧 schema 中间产物。
...
design/
  ...
  knowledge/               知识层：选择 Skills/SOP，生成 Design Skills Brief
...
当前设计理念是：LLM 负责“语义提取和文档表达”，程序规则负责“证据收集和可复现流程”。
...
知识层。负责把业务 skill 变成 Design 可用上下文。

常见改动：

- 调整 skill 打分规则
- 调整 selected skill 数量
- 调整 brief 中哪些业务规则要暴露给 writer
