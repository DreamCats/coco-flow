...
`design-research-summary.json` 记录代码证据摘要，`design-quality.json` 记录程序质量 gate，
...
7. `writer.write_doc_only_design_draft()`
   先生成本地草稿，再在 native 可用时让 writer agent 编辑 Markdown。
   native 失败时保留本地草稿。

8. `quality.build_design_quality_payload()`
   对 writer 草稿执行确定性 actionability 检查，并生成结构化质量报告。

9. `supervisor.supervisor_review_design()`
...
10. `shared.contracts.build_design_contracts_payload()`
