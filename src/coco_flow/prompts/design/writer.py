...
def build_doc_only_design_prompt(
    *,
    ...
    return (
        ...
        "design.md 必须可直接进入研发评审，不能只复述核心改造点或候选文件。\n"
        "必须保留或补齐这些章节：结论、核心改造点、分仓库方案、验收与验证、风险与待确认、明确不做。\n"
        "每个需要改造的仓库都必须写清：职责判断、代码线索、改造方案、边界。\n"
        "改造方案必须覆盖 refined PRD 的关键验收约束，例如实验命中条件、本地化取值、空值回退、分隔符、未命中实验保持不变等；如果某项不适用，写明原因。\n"
        "如果 repo research 只有相关性证据而不能证明必须修改，应写成待确认或 validate_only，不要把候选文件包装成确定改造点。\n\n"
        f"## 任务标题\n{title}\n\n"
