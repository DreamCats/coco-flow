...
def _build_skill_policy_section(skills_index_markdown: str) -> PromptSection:
    ...
    return PromptSection(
        ...
        body=(
            ...
            "如果索引中列出具体 skill 文件路径，写 design 或提取搜索线索前必须读取完整文件内容。\n"
            "读取后只采信其中的稳定规则、术语、repo role、Main Flow、Change Flows 和 SOP，不得扩写 refined PRD。\n\n"
            f"{skill_index}"
        ),
    )
...
def _build_file_io_section() -> PromptSection:
    return PromptSection(
        ...
        body="\n".join(
            [
                "1. 只读取任务 prompt 或 Skills 索引明确列出的 Markdown 文档、代码证据或 skill 文件。",
