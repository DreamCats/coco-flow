...
class PrdRunCommandTest(unittest.TestCase):
    def test_prd_run_reports_design_before_plan_and_code(self) -> None:
        runner = CliRunner()
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch("coco_flow.cli.design_task", return_value="designed") as design_task_mock,
                patch("coco_flow.cli.plan_task", return_value="planned") as plan_task_mock,
                patch("coco_flow.cli.read_task_complexity", return_value="简单"),
                patch("coco_flow.cli.code_task", return_value="coded") as code_task_mock,
            ):
                result = runner.invoke(app, ["prd", "run", "-i", "需求描述"])

            self.assertEqual(result.exit_code, 0, msg=result.output)
            self.assertLess(result.output.index("refine: refined"), result.output.index("design: designed"))
            self.assertLess(result.output.index("design: designed"), result.output.index("plan: planned"))
            self.assertLess(result.output.index("plan: planned"), result.output.index("code: coded"))
            create_task_mock.assert_called_once()
            refine_task_mock.assert_called_once()
            design_task_mock.assert_called_once()
            plan_task_mock.assert_called_once()
            code_task_mock.assert_called_once()
    ...
    def test_prd_run_stops_before_code_when_plan_is_complex(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("design: designed", result.output)
